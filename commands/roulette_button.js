...
//     // collect all members currently in a voice channel (not bots)
//     const voiceMembers = guild.members.cache.filter(m => m.voice.channel && !m.user.bot);

//     // 🧠 FIX: check first if there’s anyone in VC at all
//     if (!voiceMembers.size) {
//       return interaction.editReply("Sir, nikogo net v voice channel (");
//     }

//     // if there ARE people in VC but the roulette user is not in VC → lock them
...
//     // pick random member from voiceMembers
...
//     await interaction.channel.send(`🎲 Ruletka poletela.`);
